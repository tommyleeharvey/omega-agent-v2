...
cd "$REPO"
TS=$(date +%Y%m%d-%H%M%S)
cp src/components/omega/LiveActivityBar.jsx "src/components/omega/LiveActivityBar.jsx.bak-$TS"

cat > src/components/omega/LiveActivityBar.jsx << 'BAREOF'
...
echo ""
echo "=== git diff ==="
git diff src/components/omega/LiveActivityBar.jsx

echo ""
echo "=== Push ==="
git add src/components/omega/LiveActivityBar.jsx
git commit -m "feat: replace cycling activity ticker with persistent Manus-style step list"
